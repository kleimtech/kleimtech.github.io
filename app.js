...
(function () {
  ...
  /**
   * Normaliza el identificador de categoría para soportar tanto nombres completos ("Tarjetas de Video")
   * como identificadores cortos ("gpu").
   */
  function normalizeCategory(cat) {
    if (!cat) return "";
    const lower = normalizeText(cat);
    if (lower.includes("tarjeta") || lower.includes("video") || lower === "gpu") return "gpu";
    if (lower.includes("procesador") || lower === "cpu") return "procesadores";
    if (lower.includes("mother") || lower.includes("placa")) return "motherboards";
    if (lower.includes("ram") || lower.includes("memoria")) return "ram";
    if (lower.includes("laptop") || lower.includes("portatil")) return "laptops";
    if (lower.includes("almacen") || lower.includes("disco") || lower.includes("ssd") || lower.includes("nvme") || lower.includes("hdd")) return "almacenamiento";
    if (lower.includes("fuente") || lower.includes("psu")) return "fuentes";
    if (lower.includes("periferic") || lower.includes("mouse") || lower.includes("teclado") || lower.includes("auricular")) return "perifericos";
    if (lower.includes("monitor") || lower.includes("pantalla")) return "monitores";
    if (lower.includes("accesorio")) return "accesorios";
    if (lower.includes("gamepad") || lower.includes("control") || lower.includes("mando")) return "gamepads";
    if (lower.includes("combo") || lower.includes("kit")) return "combos";
    return lower;
  }
  ...
  function applyFilters() {
    ...
    const filtered = productList.filter(item => {
      ...
      // 3. Filtro por subcategoría (Mega Menu)
      if (state.selectedSubcategory) {
      ...
      // 4. Filtro por texto de búsqueda (nombre, specs, categoría, subcategoría, estado)
